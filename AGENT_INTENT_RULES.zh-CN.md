...
3. 解析精确实体。
4. 展示一个动作卡片。
5. 只有在经纪人确认或明确点击动作后才执行。

自然语言分析应该放在 Agnes 的文字回复里，不应该塞进动作卡片。卡片只展示确定性字段、已确认实体，以及当前这一步能做的单一选择或动作。一个卡片不能承载完整嵌套流程。

经纪人点击动作后，上一张卡片就视为完成，不应继续用于重复决策或重复写入。Agent 应继续输出新的自然语言消息，并在需要时展示新的单用途卡片。

如果经纪人只上传或粘贴 WhatsApp 聊天记录，没有明确说明要做什么，默认第一步是：先用自然语言分析对话，并立即生成一个回复卡片。起草回复是经纪人最常见的第一需求，而且不会写入业务状态；跟进管理是在线索解析后进入的独立下一步。

WhatsApp 聊天导入必须先和现有线索库做比对，再提出跟进动作：

1. 从聊天中提取客户身份信号，例如姓名、手机号、房源链接、区域、预算和买家表达。
2. 搜索当前 broker 名下的线索，先找精确匹配和高置信度匹配。
3. 如果找到已有线索，必须读取这条线索当前状态和历史进展，包括 status、urgency、last note、last contacted time、next follow-up time、感兴趣区域/房源、预算，以及最近 follow-up activities。
4. 在推荐下一步动作前，必须把新聊天和已有线索状态做对比。Agnes 应说明：这次聊天新增了什么、发生了什么变化、哪些仍需要确认。
5. 如果没有匹配线索，才把它当作首次接触对话，进入普通回复草稿和是否新建线索流程。

这个历史比对层才是“跟进智能助手”和普通聊天总结器的区别。当已有线索历史可用时，Agent 不应只根据新聊天单独推荐状态变更、提醒或保存跟进备注。

如果经纪人随聊天记录一起给出明确指令，应直接进入对应动作：

- “帮我回复” 进入回复草稿卡片。
- “保存为跟进” 进入跟进备注确认卡片。
- “设置提醒” 进入提醒确认卡片。
- “更新这个客户状态” 进入状态确认卡片。
- “分析一下” 只返回自然语言分析，不写入数据库。

所有卡片都应该可复用。例如识别到已有线索时，无论它出现在聊天导入、实体解析、设置提醒还是状态更新流程中，都应使用一致的线索摘要卡样式。

下一步建议必须是选择性的，不能是固定菜单。Agent 应只展示由聊天内容和线索历史共同支持的最高概率动作。例如：只有聊天提到时间、看房或回访请求时才展示提醒；只有出现强烈感兴趣/不感兴趣信号时才展示状态更新；只有聊天增加了有意义的新跟进信息时才展示保存备注。其他小概率动作应通过用户自然语言指令触发。

当建议动作是更新线索状态时，应以 Yes/No 的建议句呈现。用户确认 Yes 后，一次点击应同时把聊天总结保存为跟进上下文，并应用建议的 status/urgency 更新。用户选择 No 时，不写入数据库，让用户继续用自然语言说明下一步。

确认卡片的标题应该就是需要用户判断的问题，而不是泛泛的大标题。卡片正文只展示将要写入数据库的确定性字段，例如当前状态、建议改为的状态、跟进记录文字和跟进时间。当字段表已经能说明动作时，应避免再放解释性段落。
...
架构说明：能力配置、引导逻辑、确认策略、实体解析策略和 action handler manifest 详见 `docs/agent-capability-architecture.zh-CN.md`。新增 Agent workflow 时，应优先从 capability registry 开始，而不是继续把临时规则写进 `AgentWorkspace.tsx` 或 prompt 字符串。
...
- 创建日程时，不应因为自然语言房源文本（例如 “DHA Phase 5 villa”）或新的/未匹配到的客户名而阻断预览；应保留文本，只有在明确选择或高置信匹配到记录时才绑定 `lead_id`/`listing_id`。
- 前端可以把 `list_schedule_events` 作为只读 `/api/events` 查询执行，并在聊天中展示日程列表。
- Schedule 工作台页面可以列表、筛选、编辑、完成和取消 broker events；默认展示近 7 天，顶部统计在移动端必须保持紧凑。
